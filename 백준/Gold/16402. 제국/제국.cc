...
int main()
{
    ...
    for(int i=0;i<M;i++){
        ...
        string city1,city2;
        int idx = 0;
        while(order[idx]!=',') city1.push_back(order[idx++]);
        idx++;
        while(order[idx]!=',') city2.push_back(order[idx++]);
        idx++;
        int k = order[idx]-'0';
        int ct1 = mp[city1], ct2 = mp[city2]; 
        if(k==1) swap(ct1,ct2); //ct2가 항상 이긴놈이됨
        bool res = dset.merge(ct1,ct2);
        if(!res){
            dset.parent[ct1] = ct2;
            dset.parent[ct2] = ct2;
        }
    ...
    set<int> survive;
    for(int i=0;i<N;i++) survive.insert(dset.find(i));
    cout<<survive.size()<<'\n';
    vector<string> ans;
    for(auto i : survive) ans.push_back(rev[i]);
