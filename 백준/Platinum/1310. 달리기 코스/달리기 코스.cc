...
ll diameter(const polygon &p){
    int n = p.size();
    int left = min_element(p.begin(),p.end())-p.begin();
    int right = max_element(p.begin(),p.end())-p.begin();
    ll ret = (p[right]-p[left]).norm2();
...
int main()
{
    ...
    vector<vector2> vv2(N);
    for(auto &i : vv2) cin>>i.x>>i.y;
    ...
    if(N==1){
        ...
        cout<<(vv2[0]-vv2[1]).norm2();
    
        return 0;
    }
    polygon p = grahamScan(vv2);
    ll x = diameter(p);
    cout<<x;
}